...
//	printf("%d %d %d\n", blockIdx.x, blockDim.x, tid);
	...
	for(unsigned int s = 1; s < blockDim.x; s *= 2) {
		int index = 2 * s * tid;
		if(index < blockDim.x) { // With strided index and non-divergent branch
			sdata[index] = (sdata[index] < sdata[index + s])? sdata[index + s] : sdata[index];
		}
		__syncthreads();
	}
...
int main(void)
{
	...
	printf("Time for the kernel time: %f\n", time);
