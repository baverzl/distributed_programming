...
void *http_request_handler(void *param)
{
    ...
    while(1) {
        ...
        if(taskAvailable) {
            if ( read_line( clnt_sock, buffer, sizeof(buffer) ) == -1 ) 
            {
            ...
            if( path[0] != '/' ) {
                send_error( clnt_sock, 400, "Bad Request", NULL, "Bad filename." ); goto escape;
            }
...
do_file:
                fp = fopen( file, "r" );
                if( fp == NULL ) {
                    send_error( clnt_sock, 403, "Forbidden", NULL, "File is protected." ); goto escape;
                }
